...
export function parsePickRef(input) {
    const s = input.trim().replace(/\/+$/, '');
    if (!s)
        return null;
    const pull = /github\.com\/([^/]+)\/([^/]+)\/pull\/(\d+)/i.exec(s);
    if (pull)
        return { kind: 'pr', number: Number(pull[3]), repo: `${pull[1]}/${pull[2]}` };
    const commit = /github\.com\/([^/]+)\/([^/]+)\/commit\/([0-9a-f]{7,40})/i.exec(s);
    if (commit)
        return { kind: 'sha', sha: commit[3].toLowerCase(), repo: `${commit[1]}/${commit[2]}` };
    const hash = /^#(\d+)$/.exec(s);
    if (hash)
        return { kind: 'pr', number: Number(hash[1]) };
    if (/^\d+$/.test(s))
        return { kind: 'pr', number: Number(s) };
    ...
    if (tree)
        return { kind: 'branch', name: tree[3], repo: `${tree[1]}/${tree[2]}` };
...
function mergeCommitOfPr(org, repo, number) {
    const out = execFileSync('gh', ['pr', 'view', String(number), '--repo', `${org}/${repo}`, '--json', 'state,mergeCommit'], { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
    ...
    if (!oid)
        throw new Error(`PR #${number} has no merge commit (rebase-merged?) — pick its commits by SHA instead`);
    return oid;
}
/**
 ...
 */
export function resolvePicks(dir, org, repoName, inputs) {
    const errors = [];
    const warnings = [];
    const picks = [];
    const merges = [];
    const seen = new Set();
    const count = (ref, exclude) => Number(git(dir, ['rev-list', '--count', ref, ...exclude.map((e) => `^${e}`)]).trim());
        ...
        if (parsed.kind === 'branch') {
            ...
            if (isAncestor(dir, ref, 'origin/staging')) {
                ...
                const landing = git(dir, [
                    ...
                    .filter(Boolean)[0];
                if (!landing) {
                    errors.push(`${input}: '${parsed.name}' was fast-forwarded into staging — pick its commits by SHA, or its PR`);
                    continue;
                }
                if (seen.has(landing))
                    continue;
            ...
            if (refExists(dir, 'origin/develop'))
                exclude.push('origin/develop');
            ...
            if (own !== beyondProd) {
                ...
                errors.push(`${input}: merging '${parsed.name}' would drag ${beyondProd - own} commit(s) of staging/develop history into production — land it on staging first, or pick exact commits`);
                continue;
            }
            ...
            warnings.push(`${parsed.name}: ${beyondProd} commit(s) have never been through staging — QC has not seen them`);
            continue;
        }
        let sha;
        try {
    ...
    picks.sort((a, b) => {
        if (a.sha === b.sha)
            return 0;
        if (isAncestor(dir, a.sha, b.sha))
            return -1;
        if (isAncestor(dir, b.sha, a.sha))
            return 1;
