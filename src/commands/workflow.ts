...
interface WorkflowOptions {
  ...
  /** Target branch for the workflow */
  branch: string;
  /** Specific workflow name (if multiple exist) */
  workflow?: string;
  /** List available workflows instead of running */
  list: boolean;
  /** Validate parameters without running */
  dryRun: boolean;
  /** Show detailed output */
  verbose: boolean;
  /** Watch the workflow run after triggering */
  watch: boolean;
...
async function executeWorkflow(
  ...
  if (!ghAvailable) {
    console.log(createErrorBox(
      ...
      'Please install gh CLI and authenticate with: gh auth login'
    ));
    process.exit(1);
  }
  ...
  if (!repo) {
    console.log(createErrorBox(
      ...
      `Usage: vast workflow <repository> --version <version> --branch <branch>\n\nAvailable repositories:\n${formatList([...VALID_REPOSITORIES])}`
    ));
    process.exit(1);
  }
