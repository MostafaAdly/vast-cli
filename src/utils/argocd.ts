...
async function argoRequest(
  ...
  if (REDIRECT_STATUSES.has(res.status)) throw new ArgoSsoWallError(ssoWallMessage(host));
...
function serverMessage(body: string, status: number): string {
  try {
...
export async function login(
  ...
  if (!res.ok) throw new Error(`argocd login failed: ${serverMessage(body, res.status)}`);

  const token = (JSON.parse(body) as { token?: string }).token;
  if (!token) throw new Error('argocd login returned no token');
  return token;
}
...
export async function userinfo(
  ...
  if (!res.ok) throw new Error(`argocd userinfo failed: ${serverMessage(body, res.status)}`);

  const parsed = JSON.parse(body) as { loggedIn?: boolean; username?: string };
...
export function rolloutDone(app: ArgoApp, tag: string): boolean {
...
function hasTag(app: ArgoApp, tag: string): boolean {
...
export async function waitForRollout(
  ...
  tag: string,
  deps: RolloutDeps,
  timing: RolloutTiming = DEFAULT_ROLLOUT_TIMING,
  isDone: (app: ArgoApp) => boolean = (app) => rolloutDone(app, tag),
): Promise<RolloutResult> {
  const start = deps.now();
  let lastState: string | null = null;
  let lastPrintedAt = 0;
  let consecutiveErrors = 0;
  let lastApp: ArgoApp | undefined;

  for (;;) {
    let current: ArgoApp | null = null;
    try {
      ...
      if (consecutiveErrors === 1) {
        deps.print(
          `  ${label}  argocd ${appName}  read failed, retrying  ${formatElapsed(deps.now() - start)}`,
        );
      }
      if (consecutiveErrors >= timing.maxConsecutiveErrors) {
        return {
          ok: false,
          elapsedMs: deps.now() - start,
          reason: `could not read argocd application ${appName}: ${
    ...
    if (current) {
      ...
      if (isDone(current)) return { ok: true, elapsedMs: elapsed, app: current };
