...
export function createSuccessBox(message: string, details?: string): string {
...
export function createErrorBox(message: string, details?: string): string {
  const content = details 
...
/** Create an info box */
export function createInfoBox(title: string, items: string[]): string {
  const content = [
...
export const log = {
  ...
  info: (message: string): void => { console.log(colors.info(message)); },
  
  /** Success message */
  success: (message: string): void => { console.log(colors.success(`✓ ${message}`)); },
  
  /** Warning message */
  warn: (message: string): void => { console.log(colors.warning(`⚠ ${message}`)); },
  
  /** Error message */
  error: (message: string): void => { console.error(colors.error(`✗ ${message}`)); },
   ...
   */
  muted: (message: string): void => { console.log(colors.muted(message)); },
  
  /** Highlighted/important info */
  highlight: (message: string): void => { console.log(colors.highlight(message)); },
  
  /** Primary brand color */
  primary: (message: string): void => { console.log(colors.primary(message)); },
  
  /** New line */
  newline: (): void => { console.log(''); },

  /** Muted text as a string, for embedding inside another line. */
  dim: (message: string): string => colors.muted(message),
};
...
export function formatTable(
  ...
  const headerRow = headers.map((h, i) => 
    colors.primary.bold(h.padEnd(options?.colWidths?.[i] ?? 15))
  ).join('  ');
  ...
  for (const row of rows) {
    const formatted = row.map((cell, i) => 
      cell.padEnd(options?.colWidths?.[i] ?? 15)
    ).join('  ');
