...
export const PLAIN_STYLE = { paint: (_tone, text) => text, link: (text) => text };
...
function markerParts(item, age, r) {
    const out = [];
    if (item.ported)
        out.push({ text: 'ported (same code)', tone: 'ported' });
    ...
    // different code, so the wording stays "not found".
        ...
        out.push({ text: `⚠ not found on ${r.other}`, tone: 'notFound' });
    // Already on its way: how long it waited is no longer the question.
        ...
        out.push({ text: '⚠ stale', tone: 'stale' });
    if ('detailsUnavailable' in item && item.detailsUnavailable)
        out.push({ text: 'details unavailable', tone: 'muted' });
    return out;
}
...
function branchLink(t, branch, tone) {
    return t.s.link(t.s.paint(tone, branch), `${t.repoUrl}/tree/${branch}`);
}
function terminalMarkers(t, item, age, r) {
    return markerParts(item, age, r)
        .map((m) => {
        if (!m.carrier)
            return `  ${t.s.paint(m.tone, m.text)}`;
...
function terminalPr(t, p, r, indent) {
    const { s } = t;
...
function terminalPrList(t, prs, r) {
    const { s } = t;
    if (!t.o.byTicket)
        return prs.flatMap((p) => terminalPr(t, p, r, '    '));
...
function terminalRepo(r, o, s) {
    const t = { o, s, repoUrl: r.repoUrl };
    if (r.problem || !r.forward)
        return [`  ${repoLink(t, r.repo)}  ${problemText(t, r.problem)}`.trimEnd()];
    const f = r.forward;
    const rv = r.reverse;
    ...
    if (itemCount(f) === 0 && (!rv || itemCount(rv) === 0))
        return [`${head}${s.paint('ok', ' · in sync')}`];
    const lines = [head];
    ...
    if (rv) {
        ...
        if (itemCount(rv) === 0)
            lines.push('', `  ${s.paint('ok', `Nothing on ${rv.source} that ${rv.target} lacks.`)}`);
        else
 ...
 */
function sweepTable(report, o, s) {
    ...
    const pad = (plain, dressed, width) => dressed + ' '.repeat(Math.max(0, width - plain.length));
    const repoCell = (i) => {
