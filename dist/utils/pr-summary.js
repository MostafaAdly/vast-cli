...
const isAcronym = (w) => w.length >= 2 && /^[A-Z0-9-]+$/.test(w) && /[A-Z]/.test(w);
 ...
 */
export function heuristicSummary(title) {
    ...
    const caseOf = (word, isFirst) => {
        if (isAcronym(word) || isCamelCase(word))
            return word;
        if (isCapitalized(word) && !isFirst && !titleCase)
            return word;
    ...
    for (const part of stripped.split(PART_SEPARATOR)) {
        ...
        for (const token of part.split(/\s+/).map(cleanToken).filter(Boolean)) {
            ...
            if (VERBS.has(lower) || ARTICLES.has(lower))
                continue;
            ...
            if (collected.length > 0 && lower.length >= 6 && lower.endsWith('ing'))
                break;
            collected.push(caseOf(token, isFirst));
            if (collected.length === MAX_PART_WORDS)
                break;
        }
...
export function buildSummaryPrompt(prs) {
    return [
 ...
 */
function execWithInput(file, args, input, timeout, env) {
    return new Promise((resolve, reject) => {
        const child = execFile(file, args, { encoding: 'utf-8', timeout, maxBuffer: 1024 * 1024, env }, (error, stdout) => error ? reject(error) : resolve(String(stdout)));
...
export async function modelPhrases(prs, deps = defaultDeps) {
    ...
    const chunks = [];
    for (let i = 0; i < prs.length; i += PRS_PER_CALL)
        chunks.push(prs.slice(i, i + PRS_PER_CALL));
