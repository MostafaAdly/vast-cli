...
function readVersionField(json: string): string | null {
  return /"version"\s*:\s*"([^"]+)"/.exec(json)?.[1] ?? null;
}

function setVersionField(json: string, version: string): string {
...
export const RELEASE_KINDS: ReleaseKind[] = ['release', 'hotfix'];

function git(dir: string, args: string[]): string {
  return execFileSync('git', args, { cwd: dir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
}

export function releaseBranchName(kind: ReleaseKind, version: string): string {
  return `${kind}/${stripRc(version)}`;
}

/** @returns the PR URL, or null if nothing was opened. */
export function cutReleaseBranch(
  dir: string,
  repo: string,
  kind: ReleaseKind,
  version: string,
  ...
  if (!trial.clean && !onlyVersionConflict) {
    ...
    for (const f of trial.conflicts) log.error(`  • ${f}`);
    return null;
  }
  ...
  if (onlyVersionConflict) {
    const stagingVersion = readVersionField(
      git(dir, ['show', `origin/staging:${GENERATED_VERSION_FILE}`]),
    );
    if (!stagingVersion) {
 ...
 * @returns the PR URL, or null if nothing was opened.
 */
export function cutPickedBranch(
  dir: string,
  repo: string,
  kind: ReleaseKind,
  version: string,
  ...
  if (!applied.ok) {
    ...
    for (const f of applied.conflicts) log.error(`  • ${f}`);
  ...
  if (!merged.ok) {
    ...
    for (const f of merged.conflicts) log.error(`  • ${f}`);
