...
export function columnWidth(rows: Row[], gutter = 2): number {
 ...
 */
function fitRight(text: string, available: number): string[] {
  const lines: string[] = [];
  let line = '';
...
export function commandRow(row: Row, width: number): string {
...
export function exampleRow(row: Row, width: number): string {
 ...
 * = red" instinct.
 */
export function lockState(state?: { ready?: boolean; enabled?: boolean }): string {
