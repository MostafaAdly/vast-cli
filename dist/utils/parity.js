...
function run(dir, args, input, env) {
    return new Promise((resolve) => {
        const child = execFile('git', args, { cwd: dir, encoding: 'utf-8', maxBuffer: 512 * 1024 * 1024, env: env ? { ...process.env, ...env } : process.env }, (error, stdout) => {
...
async function git(dir, args, input) {
    ...
    if (r.code !== 0)
        throw new Error(`git ${args[0]} failed (exit ${r.code})`);
    return r.stdout;
}
...
async function commitsIn(dir, base, head) {
...
function ancestorsInRange(start, commits) {
    const seen = new Set();
    const stack = [start];
 ...
 */
function splitBySha(out, wanted) {
    const patches = new Map();
    let current = null;
    let lines = [];
    const flush = () => {
        if (current)
            patches.set(current, lines.join('\n') + (lines.length > 0 ? '\n' : ''));
    };
 ...
 */
async function patchesOf(dir, items, extraArgs = [], paths = []) {
    if (items.length === 0)
        return new Map();
    const input = items.map((i) => (i.base ? `${i.sha} ${i.base}` : i.sha)).join('\n') + '\n';
    const out = await git(dir, ['diff-tree', '--stdin', '--root', '-p', '--no-color', '--no-ext-diff', ...extraArgs, ...(paths.length ? ['--', ...paths] : [])], input);
    const patches = splitBySha(out, new Set(items.map((i) => i.sha)));
    for (const i of items)
        if (!patches.has(i.sha))
            patches.set(i.sha, '');
    return patches;
}
/** Patch-ids of patches already in hand, in one pipe. */
async function patchIdsOf(dir, patches) {
    const ids = new Map();
 ...
 */
async function bookkeepingShas(dir, candidates) {
    const out = new Set();
    const versionOnly = [];
    for (const c of candidates) {
        if (c.files.length === 0)
            continue;
        if (c.files.every((f) => HELM_VALUES.test(f)))
            out.add(c.sha);
    ...
    for (const [sha, diff] of diffs) {
        ...
        if (changed.length > 0 && changed.every((l) => /"version"\s*:/.test(l)))
            out.add(sha);
    }
    return out;
}
async function readSide(dir, base, head) {
    ...
    for (const c of list) {
        ...
        if (c.parents.length >= 2) {
            ...
            for (const sha of ancestorsInRange(c.parents[1], commits))
                if (!mainline.has(sha))
                    owned.add(sha);
        }
        const unit = { number: pr.number, branch: pr.branch, sha: c.sha, landedAt: c.landedAt, patchId: null };
    ...
    const candidates = list.filter((c) => c.parents.length < 2 && // PR merges handled above; other merges carry no work of their own
 ...
 */
async function containedPatches(dir, ref, patches) {
    const found = new Set();
    const entries = [...patches].filter(([, p]) => p.trim());
    if (entries.length === 0)
        return found;
    await withIndexOf(dir, ref, (env) => mapLimit(entries, APPLY_CONCURRENCY, async ([sha, patch]) => {
 ...
 */
export async function containedIn(dir, ref, shas) {
    if (shas.length === 0)
        return new Set();
 ...
 */
async function duplicatesInHistory(dir, ref, items) {
    const found = new Set();
    if (items.length === 0)
        return found;
 ...
 */
async function portedOf(dir, side, items, otherItems, otherRef, patches) {
    const otherIds = new Set(otherItems.map((i) => i.patchId).filter((p) => p !== null));
    ...
    for (const sha of await duplicatesInHistory(dir, otherRef, plain))
        ported.add(sha);
    const rest = new Map(items.filter((i) => !ported.has(i.sha)).map((i) => [i.sha, patches.get(i.sha) ?? '']));
    for (const sha of await containedPatches(dir, otherRef, rest))
        ported.add(sha);
    return ported;
}
function diffBase(side, sha) {
    const c = side.commits.get(sha);
    return c && c.parents.length >= 2 ? { sha, base: c.parents[0] } : { sha };
}
export async function compareBranches(dir, source, target) {
    ...
    for (const i of srcItems)
        i.patchId = srcIds.get(i.sha) ?? null;
    for (const i of tgtItems)
        i.patchId = tgtIds.get(i.sha) ?? null;
