 ...
 * here looks at.
 */
export function refspecsFor(branches) {
 ...
 */
export async function fetchBranches(dir, branches) {
    const run = (refs) => execFileAsync('git', ['fetch', '--no-tags', 'origin', ...refspecsFor(refs)], {
        cwd: dir,
        encoding: 'utf-8',
    });
    try {
 ...
 * resolves.
 */
export async function fetchExactly(dir, branches) {
    try {
 ...
 */
export function mergeSequence(dir, refs) {
