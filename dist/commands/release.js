 ...
 */
import { reposForRelease } from '../config/repos.js';
...
// than written twice.
export { isSweep, pollIntervalFor, pollTimingFor };
...
export async function prepareOne(repo, options) {
    ...
    if (!dir)
        return notClonedOutcome(repo.name, isSweep(options));
 ...
 */
export async function releaseMany(targets, options, deps = {}) {
    const prepare = deps.prepare ?? prepareOne;
    const planned = [];
    for (const repo of targets)
        planned.push(await prepare(repo, options));
