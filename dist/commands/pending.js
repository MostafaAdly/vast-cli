...
import { compareBranches } from '../utils/parity.js';
import { ghPrLookup, prNumbersInRange, resolveMentions } from '../utils/shipped.js';
...
async function collectOne(repo, to, opts, sweep, deps) {
    ...
    if (!source)
        return problem('skipped', 'no develop branch');
    ...
    const openReleases = heads.map((h) => ({
        ...h,
