...
export function sweepAndNamesProblem(names: string[], sweep: Sweep): string | null {
 ...
 */
export function perRepoOptionProblem(
  names: string[],
 ...
 */
export function versionFor(env: DeployEnv, stagingTag: string): string {
...
export const DEFAULT_DEPLOY_DEPS: DeployDeps = {
  runWorkflow,
  getRunStatus,
  failedStepName,
  waitForRollout,
...
export async function deployOne(
  ...
  deps: DeployDeps = DEFAULT_DEPLOY_DEPS,
  ...
  const outcome = (status: DeployOutcome['status'], detail: string): DeployOutcome => ({
    repo: repo.name,
    version,
    status,
    detail,
...
export async function deployMany(
  planned: Array<Deployable | DeployOutcome>,
  env: DeployEnv,
  dryRun: boolean,
  deps: DeployManyDeps = {},
): Promise<DeployOutcome[]> {
  const live = process.stdout.isTTY === true;
  const going = planned.filter((p): p is Deployable => !isOutcome(p));
...
interface DeployOptions extends Sweep {
  to: DeployEnv;
  dir?: string;
