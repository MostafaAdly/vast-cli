...
function joinCapped(items: string[]): string {
  ...
  if (items.length > MAX_ITEMS) kept.push('…');
  return kept.join(', ');
}
 ...
 */
export function describe(prs: ShippedPr[], fallbackSubjects: string[]): string {
  const items: string[] = [];
  ...
  if (prs.length > 0) {
    for (const pr of prs) push(tidy(parseSubject(pr.title).text));
  } else {
    for (const subject of fallbackSubjects) push(tidy(subject));
  }
 ...
 */
function authors(prs: ShippedPr[], mentions: Record<string, string | null>): string[] {
  const out: string[] = [];
...
export function buildReleaseMessage(input: ReleaseMessageInput): string {
