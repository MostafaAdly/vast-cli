...
export function parseRemote(url: string): RemoteId | null {
...
export function canonicalRepoName(url: string): string | null {
  const id = parseRemote(url);
  if (!id) return null;
  if (id.owner.toLowerCase() !== ORG.toLowerCase()) return null;
