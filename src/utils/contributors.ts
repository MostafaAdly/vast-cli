 ...
 */
export const EXCLUDED_CONTRIBUTORS: string[] = [
  'mahmoudelzahaby',
  'mahmoudelzahabi',
  'alielhabal',
  'youssifelzahaby',
...
export function contributorKey(c: Contributor): string {
 ...
 */
function isBot(value: string): boolean {
  const raw = value.trim().toLowerCase();
  if (!raw) return false;
  if (raw.endsWith('[bot]') || raw.startsWith('app/')) return true;
 ...
 */
export function mergeContributors(lists: Contributor[][]): Contributor[] {
  ...
  for (const c of lists.flat()) {
    ...
    if (!seen.name.trim() && c.name.trim()) seen.name = c.name;
    if (!seen.login && c.login) seen.login = c.login;
    for (const email of c.emails) if (!seen.emails.includes(email)) seen.emails.push(email);
  }
